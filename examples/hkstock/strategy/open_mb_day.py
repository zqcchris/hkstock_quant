...
from utils.myta.ta_test import MACD
...
class Critic(object):
    ...
    def validation(self, bar_5m):
        if self.signal_type == "w":
            ...
            closes = Series([float(b.close) for b in bars])
            macd, diff, dea = MACD(close=closes)
    ...
    def open(self, bar_5m):
        if self.signal_type == "w":
            ...
            """ 上轨滚仓空单 """
            bars = self.bars_generator.get_bars(symbol=self.symbol, period=Interval.min15.value, end_time=bar_5m.dt,
                                                limit=50)
            if not bars or len(bars) < 21: return bars
            closes = Series([float(b.close) for b in bars])
            boll = BollingerBands(close=closes, window=21)
...
class Strategy(object):
    ...
    def run(self, symbol, step_interval, bar_5m=None, mode=""):
        ...
        print(self.critic)
        self.critic.trigger_generation(symbol=symbol, bar_5m=bar_5m)
        print(self.critic.trigger)

        if mode == "init": return
        self.position_cache.cache_update(symbol=symbol, cache=self.cache, bar_5m=bar_5m)

        o, psd, trigger_time, cat, interval = self.open(symbol=symbol, bar_5m=bar_5m)
        if not psd: return
        if cat == "normal":
            ...
            if trigger_time not in self.olds:
                ...
                sl_price, sl_rate = self.sl_algo(psd=psd, bar_5m=bar_5m, bars_4h=bars_4h, open_price=o,
                                                 interval=interval)
                position = MockPosition(symbol=symbol, position_side=psd, open_price=o, precision=self.precision,
                                        mode=self.mode, open_time=bar_5m.dt, interval=interval,
                                        step_interval=step_interval,
                                        comment=interval, uuid=uuid.uuid1(), sl_price=sl_price, sl_rate=sl_rate,
                                        trigger_time=trigger_time, category=cat)
                self.update_db(position=position)
    ...
    def critic_generation(self, symbol, bar_5m=None):
